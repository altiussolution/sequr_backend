...
//   }
// }
exports.createRole = (req, res) => { // Change your function name
  var newRole = new rolesModel(req.body) // Change model name 
  newRole.save((err, doc) => { // past model body
    if (!err) {
      res
        .status(200)
    ...
    rolesModel //(paste your model)
      ...
      .then(async doc => {
        ...
        if (doc[0].user_doc.length > 0) {
          await message.push('Please delete all the refered users by this role')
        }
        // *** 2 ***
