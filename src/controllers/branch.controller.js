...
exports.getBranch = (req, res) => {
    branchModel.find({active_status: 1}).populate("state_id").populate("city_id").exec((err, branch) => {
        if (!err) {
...
exports.deleteBranch = (req, res) => {
    branchModel.findByIdAndUpdate(req.params.id, {active_status: 0} , function(err, branch){
        if (!err) {
